...
def run_inference_on_image(image_path, model, query, use_tiling=False):
    ...
    if not use_tiling:
        ...
        return proc.process_detections(detections, query, image), image
    else:
        ...
        return proc.process_detections(det_all, query, image), image
...
if __name__ == "__main__":
    try:
        ...
        (result, img) = run_inference_on_image(SOURCE_IMAGE_PATH, grounding_dino_model, QUERY, use_tiling=USE_TILING)
        if result:
            ...
            base_dir = ensure_dir(os.path.join(HOME, "results", f"{img_name}_{timestamp}"))
